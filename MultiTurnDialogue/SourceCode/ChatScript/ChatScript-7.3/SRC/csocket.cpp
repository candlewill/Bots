...
//   ChatScriptSocketException Code

ChatScriptSocketException::ChatScriptSocketException(const string &message, bool inclSysMsg)
...
ChatScriptSocketException::~ChatScriptSocketException() throw() {}
const char *ChatScriptSocketException::what() const throw() { return userMessage.c_str();}
...
static void fillAddr(const string &address, unsigned short port, sockaddr_in &addr) {
  ...
  if ((host = gethostbyname(address.c_str())) == NULL)  throw ChatScriptSocketException((char*)"Failed to resolve name (gethostbyname())");
...
CSocket::CSocket(int type, int protocol) throw(ChatScriptSocketException) {
  #ifdef WIN32
	if (InitWinsock() == FAILRULE_BIT)  throw ChatScriptSocketException((char*)"Unable to load WinSock DLL");  //   Load WinSock DLL
  #endif

  //   Make a new socket
  if ((sockDesc = socket(PF_INET, type, protocol)) < 0)  throw ChatScriptSocketException((char*)"Socket creation failed (socket())", true);
}
...
string CSocket::getLocalAddress() throw(ChatScriptSocketException) {
  sockaddr_in addr;
  unsigned int addr_len = sizeof(addr);
  if (getsockname(sockDesc, (sockaddr *) &addr, (socklen_t *) &addr_len) < 0)  throw ChatScriptSocketException((char*)"Fetch of local address failed (getsockname())", true);
  return inet_ntoa(addr.sin_addr);
}

unsigned short CSocket::getLocalPort() throw(ChatScriptSocketException) {
  sockaddr_in addr;
  unsigned int addr_len = sizeof(addr);
  if (getsockname(sockDesc, (sockaddr *) &addr, (socklen_t *) &addr_len) < 0)  throw ChatScriptSocketException((char*)"Fetch of local port failed (getsockname())", true);
  return ntohs(addr.sin_port);
}

void CSocket::setLocalPort(unsigned short localPort) throw(ChatScriptSocketException) {
#ifdef WIN32
	if (InitWinsock() == FAILRULE_BIT) throw ChatScriptSocketException((char*)"Unable to load WinSock DLL");
#endif
	...
	if (::bind(sockDesc, (sockaddr *) &localAddr, sizeof(sockaddr_in)) < 0) throw ChatScriptSocketException((char*)"Set of local port failed (bind())", true);
}

void CSocket::setLocalAddressAndPort(const string &localAddress,
    unsigned short localPort) throw(ChatScriptSocketException) {
	...
	setsockopt(sockDesc, SOL_SOCKET, SO_REUSEADDR, (void*) &off, sizeof(on));
#endif
	if (::bind(sockDesc, (sockaddr *) &localAddr, sizeof(sockaddr_in)) < 0)  throw ChatScriptSocketException((char*)"Set of local address and port failed (bind())", true);
}

void CSocket::cleanUp() throw(ChatScriptSocketException) {
  #ifdef WIN32
    if (WSACleanup() != 0)  throw ChatScriptSocketException((char*)"WSACleanup() failed");
  #endif
}
...
CommunicatingSocket::CommunicatingSocket(int type, int protocol)  throw(ChatScriptSocketException) : CSocket(type, protocol) {
}

CommunicatingSocket::CommunicatingSocket(int newConnSD) : CSocket(newConnSD) {
}

void CommunicatingSocket::connect(const string &foreignAddress,unsigned short foreignPort) throw(ChatScriptSocketException) {
  //   Get the address of the requested host
  sockaddr_in destAddr;
  fillAddr(foreignAddress, foreignPort, destAddr);

  //   Try to connect to the given port
  if (::connect(sockDesc, (sockaddr *) &destAddr, sizeof(destAddr)) < 0)   throw ChatScriptSocketException((char*)"Connect failed (connect())", true);
}

void CommunicatingSocket::send(const void *buffer, int bufferLen) throw(ChatScriptSocketException) {
  if (::send(sockDesc, (raw_type *) buffer, bufferLen, 0) < 0)  throw ChatScriptSocketException((char*)"Send failed (send())", true);
}

int CommunicatingSocket::recv(void *buffer, int bufferLen) throw(ChatScriptSocketException) {
  int rtn;
  if ((rtn = ::recv(sockDesc, (raw_type *) buffer, bufferLen, 0)) < 0)  throw ChatScriptSocketException((char*)"Received failed (recv())", true);
  return rtn;
}

string CommunicatingSocket::getForeignAddress()  throw(ChatScriptSocketException) {
  sockaddr_in addr;
  unsigned int addr_len = sizeof(addr);
  if (getpeername(sockDesc, (sockaddr *) &addr,(socklen_t *) &addr_len) < 0)  throw ChatScriptSocketException((char*)"Fetch of foreign address failed (getpeername())", true);
  return inet_ntoa(addr.sin_addr);
}

unsigned short CommunicatingSocket::getForeignPort() throw(ChatScriptSocketException) {
  sockaddr_in addr;
  unsigned int addr_len = sizeof(addr);
  if (getpeername(sockDesc, (sockaddr *) &addr, (socklen_t *) &addr_len) < 0)  throw ChatScriptSocketException((char*)"Fetch of foreign port failed (getpeername())", true);
  return ntohs(addr.sin_port);
}

//   TCPSocket Code

TCPSocket::TCPSocket() throw(ChatScriptSocketException) : CommunicatingSocket(SOCK_STREAM, IPPROTO_TCP) {
}

TCPSocket::TCPSocket(const string &foreignAddress, unsigned short foreignPort) throw(ChatScriptSocketException) : CommunicatingSocket(SOCK_STREAM, IPPROTO_TCP) {
...
TCPServerSocket::TCPServerSocket(unsigned short localPort, int queueLen) throw(ChatScriptSocketException) : CSocket(SOCK_STREAM, IPPROTO_TCP) {
  setLocalPort(localPort);
  setListen(queueLen);
}

TCPServerSocket::TCPServerSocket(const string &localAddress, unsigned short localPort, int queueLen) throw(ChatScriptSocketException) : CSocket(SOCK_STREAM, IPPROTO_TCP) {
  setLocalAddressAndPort(localAddress, localPort);
  setListen(queueLen);
}

TCPSocket *TCPServerSocket::accept() throw(ChatScriptSocketException) {
  int newConnSD;
  if ((newConnSD = ::accept(sockDesc, NULL, 0)) < 0)  throw ChatScriptSocketException((char*)"Accept failed (accept())", true);
  return new TCPSocket(newConnSD);
}

void TCPServerSocket::setListen(int queueLen) throw(ChatScriptSocketException) {
  if (listen(sockDesc, queueLen) < 0)  throw ChatScriptSocketException((char*)"Set listening socket failed (listen())", true);
}
...
SOURCE: 
	...
	catch(ChatScriptSocketException e) { myexit((char*)"failed to connect to server\r\n");}
}
#endif
...
static void* AcceptSockets(void*) // accepts incoming connections from users
{
   try {
	...
	} catch (ChatScriptSocketException &e) {printf((char*)"%s",(char*)"accept busy\r\n"); ReportBug((char*)"***Accept busy\r\n")}
    myexit((char*)"end of internet server");
	return NULL;
}
...
static void* HandleTCPClient(void *sock1)  // individual client, data on STACK... might overflow... // WINDOWS + LINUX
{
	...
	*IP = 0;
    ...
    } catch (ChatScriptSocketException e)
	{ 
...
void GrabPort() // enable server port if you can... if not, we cannot run. 
{
    ...
    catch (ChatScriptSocketException &e) {exit(1);} // dont use myexit. dont want log entries for failed port used by cron to try to restart server which may be running ok.
	echo = 1;
